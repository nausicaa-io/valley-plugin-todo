...
import { registerTodoFence } from '../src/fence'
  ...
  it('owns and disposes its stylesheet in every theme', () => {
    for (const theme of ['dark', 'light']) {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        expect(document.querySelectorAll('#notes-todo-styles')).toHaveLength(1)
        dispose()
        expect(document.querySelectorAll('#notes-todo-styles')).toHaveLength(0)
      }
    }
  })
})
...
it('disposes the lazily mounted fence stylesheet', () => {
  ...
  const dispose = registerTodoFence()
  expect(document.getElementById('notes-todo-fence-styles')).toBeNull()
  mock.codeBlockRenderers.get('todo')?.('', document.createElement('div'), { path: null, meta: null })
  expect(document.querySelectorAll('#notes-todo-fence-styles')).toHaveLength(1)
  dispose()
  expect(document.getElementById('notes-todo-fence-styles')).toBeNull()
})
  it('keeps a bottom separator under the final flat Todo row', () => {
    const disposeTodo = injectStyles()
    const todoCss = document.getElementById('notes-todo-styles')?.textContent ?? ''
    expect(todoCss).toMatch(/\.todo-panel \.todo-list-rows > \.todo-swipe:last-child \.todo-row \{\s*box-shadow: inset 0 -1px 0 var\(--border-light\)/)
    disposeTodo()
